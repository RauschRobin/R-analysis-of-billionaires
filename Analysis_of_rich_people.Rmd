...
ggplot(df_temp, aes(x = reorder(personName, -finalWorth), y = finalWorth, fill = personName)) + # nolint: line_length_linter, line_length_linter.
    ...
    geom_text(aes(label = finalWorth), vjust = -0.5, color = "black", size = 2) + # Add text labels # nolint: line_length_linter.
    labs(
...
# Konvertiere 'birthDate' in Datumsformat
df_temp$birthDate <- as.Date(df_temp$birthDate, format = "%m/%d/%Y")

# Kürze die Spaltennamen
colnames(df_temp) <- c(
    "Rank", "Worth", "Category", "Name", "Age", "Country", "City", "Source",
    "Industries", "Citizenship", "Organization", "SelfMade", "Status", "Gender",
    "BirthDate", "LastName", "FirstName", "Title", "Date", "State",
    "ResidenceRegion", "BirthYear", "BirthMonth", "BirthDay", "CPI",
    "CPIChange", "GDP", "TertiaryEnrollment", "PrimaryEnrollment",
    "LifeExpectancy", "TaxRevenue", "TotalTaxRate", "Population", "Latitude",
    "Longitude"
)

# Filtere nur numerische Spalten für die Korrelationsanalyse
numeric_cols <- sapply(df_temp, is.numeric)
df_temp_numeric <- df_temp[, numeric_cols]

# Entferne die Spalte 'Rank'

df_temp_numeric <- df_temp_numeric[, !colnames(df_temp_numeric) %in% c("Rank")]

# Entferne NA- und NaN-Werte
df_temp_numeric <- na.omit(df_temp_numeric)

# Plotte die Korrelationsmatrix
ggcorrplot(cor(df_temp_numeric), type = "lower", lab = TRUE, lab_size = 2)
```
...
```{r}
cat("Number of billionaires owning more than 50% of total wealth of billionaires:", num_billionaires_50_percent, "\n")
```
...
ggplot(wealth_filtered, aes(x = finalWorth, y = industries)) +
    geom_point() +
    labs(
...
```{r}
barplot(industry_plot_data$average_wealth,
    names.arg = industry_plot_data$industry,
...
ggplot(wealth_filtered, aes(x = reorder(category, -finalWorth), y = finalWorth, fill = selfMade)) +
...
ggplot(gdp_wealth_data, aes(x = gdp_country, y = finalWorth)) +
    geom_point() +
    labs(
        title = "Relationship between GDP and Billionaire Wealth",
        x = "GDP (in trillions USD)",
        y = "Billionaire Wealth (in billions USD)"
    )
```

There is no obvious correlation or trend.
Now let's look at the relationship between tax revenue and billionaires wealth:

```{r}
ggplot(gdp_wealth_data, aes(x = tax_revenue_country_country, y = finalWorth)) +
    geom_point() +
    labs(
        title = "Relationship between Tax Revenue and Billionaire Wealth",
        x = "Tax Revenue (in billions USD)",
        y = "Billionaire Wealth (in billions USD)"
    )
```

This plot shows that there is no obvious correlation or trend between a countries tax revenue and the billionaires wealth.
Last but not least, let's look at the relationship between a countires population and the billionaires wealth:

```{r}
ggplot(gdp_wealth_data, aes(x = population_country, y = finalWorth)) +
    geom_point() +
    labs(
        title = "Relationship between Population and Billionaire Wealth",
        x = "Population",
        y = "Billionaire Wealth (in billions USD)"
    )
```

Again, the plot shows no obvious correlation or trend.
All these plots are biased by the amount of billionaires of each country. You can always identify the usa, france and china. The differences between those countries amost always are responsible for the fact that you cannot identify a correlation or trend.
...
plot_ly(
    type = "choropleth",
    locations = df_geographical$country,
    locationmode = "country names",
    z = df_geographical$count,
    text = df_geographical$country,
    colorscale = "Turbo",
...
```{r}
for (i in 1:5) {
    cat(df_geographical[i, ]$country, ":", df_geographical[i, ]$count, "\n")
}
```
...
plot_ly(
    type = "choropleth",
    locations = df_worth_country$country,
    locationmode = "country names",
    z = df_worth_country$total_finalWorth,
    text = df_worth_country$country,
    colorscale = "Turbo",
...
ggplot(df_inherit_country, aes(x = country, y = count, fill = factor(selfMade))) +
...
```{r}
# Scatter plot: Wealth vs. Age
ggplot(df_final_worth_below_25b, aes(x = age, y = finalWorth, color = selfMade)) +
    geom_point() +
    labs(
        title = "Wealth vs. Age",
        x = "Age",
...
ggplot(df_final_worth_below_25b, aes(x = age, fill = factor(selfMade))) +
...
```{r}
# Create a bar plot
ggplot(df_age_groups, aes(x = age_group, fill = factor(selfMade))) +
...
* Is billionaire wealth concentrated among a small number of people?
* Is there a correlation between industries and GDP?
* Does the geographic distribution of billionaires reflect economic factors?
* Is there a correlation between age and being self-made?
* Do countries with lower taxes have more billionaires?
* Is there a link between education and number of billionaires?

The main findings were:

* Just over 1,100 billionaires own 80% of total billionaire wealth, indicating high concentration. However, billionaires only own 2.7% of total global wealth.
* There was no clear correlation found between industries and GDP. The technology industry had the highest average wealth per billionaire.
* The US, China, and India had the most billionaires, likely reflecting their large populations and economic power. However, no industry correlations were found.
* The youngest self-made billionaire was 28 years old. Older billionaires were more likely to have inherited their wealth.
* Countries with higher tax rates had slightly more billionaires, countering the hypothesis.
* A moderate correlation (0.25) was found between tertiary education enrollment and number of billionaires per country. No correlation was found for primary education.
