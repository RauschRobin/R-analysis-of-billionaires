...
data <- df_billionaires
data$finalWorth <- as.numeric(gsub("[^0-9.]", "", data$finalWorth))
data$gdp_country <- as.numeric(gsub("[^0-9.]", "", data$gdp_country))
data$tax_revenue_country_country <- as.numeric(gsub("[^0-9.]", "", data$tax_revenue_country_country))
data$population_country <- as.numeric(gsub("[^0-9.]", "", data$population_country))

# Filter out rows with missing values in relevant columns
data <- data %>% filter(!is.na(finalWorth) & !is.na(gdp_country))

# Create scatter plots
ggplot(data, aes(x = gdp_country, y = finalWorth)) +
  geom_point() +
  labs(title = "Relationship between GDP and Billionaire Wealth",
...
```{r}
ggplot(data, aes(x = tax_revenue_country_country, y = finalWorth)) +
  geom_point() +
  labs(title = "Relationship between Tax Revenue and Billionaire Wealth",
...
```{r}
ggplot(data, aes(x = population_country, y = finalWorth)) +
  geom_point() +
  labs(title = "Relationship between Population and Billionaire Wealth",
       x = "Population",
       y = "Billionaire Wealth (in billions USD)")
```
