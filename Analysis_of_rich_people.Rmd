...
df_temp <- df_billionaires[df_billionaires$selfMade == FALSE, ]

# Country must not be ""
df_temp <- df_temp[df_temp$country != "", ]
df_temp <- df_temp[!is.na(df_temp$country), ]

# Print out Country with most inherited billionaires
cat("The country with the most inherited billionaires is", names(which.max(table(df_temp$country))), "with", max(table(df_temp$country)), "inherited billionaires.") # nolint: line_length_linter.
```
```{r}
# use df_billionaires and corellate selfMade with everythin
df_temp <- df_billionaires

# drop rank, personName, source, industries, countryOfCitizenship, organization, lastName, firstName, birthDate, title, date, cpi_country, cpi_change_country, gdp_country, gross_tertiary_education_enrollment, gross_primary_education_enrollment_country, life_expectancy_country, tax_revenue_country_country, total_tax_rate_country, population_country, latitude_country, longitude_country
df_temp <- df_temp[, !colnames(df_temp) %in% c("rank", "personName", "source", "industries", "countryOfCitizenship", "organization", "lastName", "firstName", "birthDate", "title", "date", "cpi_country", "cpi_change_country", "gdp_country", "gross_tertiary_education_enrollment", "gross_primary_education_enrollment_country", "life_expectancy_country", "tax_revenue_country_country", "total_tax_rate_country", "population_country", "latitude_country", "longitude_country", "residenceStateRegion")]

# convert selfMade to numeric
df_temp$selfMade <- as.numeric(df_temp$selfMade)

# convert city to factor
df_temp$city <- as.factor(df_temp$city)

# convert category to factor
df_temp$category <- as.factor(df_temp$category)

# convert country to factor
df_temp$country <- as.factor(df_temp$country)

# convert state to factor
df_temp$state <- as.factor(df_temp$state)

# convert status to factor
df_temp$status <- as.factor(df_temp$status)

# convert gender to factor
df_temp$gender <- as.factor(df_temp$gender)
...
```{r}
# Bar chart: Wealth by Industry
ggplot(df_billionaires, aes(x = reorder(category, -finalWorth), y = finalWorth, fill = selfMade)) +
...
ggplot(df_billionaires, aes(x = gender, y = finalWorth, fill = gender)) +
    geom_boxplot() +
    labs(
...
ggplot(df_temp, aes(x = country, y = totalWorth, fill = selfMade)) +
    geom_bar(stat = "identity") +
    labs(
